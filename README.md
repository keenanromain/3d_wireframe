...
The last key function is the 'run_bresenham' function which is the last function that performs any calculations before the coordinates are outputted onto the graphics window. This function is the generic Bresenham Line Drawing Algorithm plus a few minor changes in order to get different colored output. This is obtained through checking if the absolute value between the x1 and x2 coordinates are less than the absolute value between the y1 and y2 coordinates within the environment struct. This calculation is stored as 'env.z' and if it is true, a red vector connecting coordinates is drawn. Otherwise, if the most current value of the 2d grid variable is a positive number, the condition of 'env.top' is met and a blue line is drawn. If neither of the previous conditions are true, then a yellow line is drawn. 
